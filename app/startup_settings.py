...
from app.ui_theme import COLORS
...
class StartupSettingsDialog:
    ...
    def show(self, language: str) -> None:
        ...
        ctk.CTkLabel(
            window,
            text=translate("startup_settings", language),
            font=("Segoe UI", 20, "bold"),
        ).grid(row=0, column=0, sticky="w", padx=24, pady=(24, 16))
        self.startup_var = tk.BooleanVar(
            master=window, value=self.startup.is_enabled(sys.executable)
        )
        self.startup_switch = ctk.CTkSwitch(
            window,
            text=translate("start_with_windows", language),
        ...
        if not self.startup.is_supported():
            ...
            ctk.CTkLabel(
                window,
                text=translate("startup_exe_only", language),
                text_color=COLORS.muted_text,
                wraplength=380,
                justify="left",
            ).grid(row=2, column=0, sticky="w", padx=24, pady=(6, 14))

        ctk.CTkLabel(window, text=translate("default_startup_mode", language)).grid(
            row=3, column=0, sticky="w", padx=24, pady=(8, 6)
        )
        self.mode_labels = {
        ...
        selected = next(
            label for label, mode in self.mode_labels.items() if mode == current
        )
        self.mode_menu = ctk.CTkOptionMenu(
            window, values=list(self.mode_labels), width=260
        )
        self.mode_menu.set(selected)
        self.mode_menu.grid(row=4, column=0, sticky="w", padx=24)
        self.opacity_var = tk.DoubleVar(
            master=window, value=load_widget_idle_opacity(self.settings_path)
        )
        ...
        ctk.CTkButton(
            window,
            text=translate("save_settings", language),
            command=self.save,
            width=130,
        ).grid(row=7, column=0, sticky="e", padx=24, pady=24)
    ...
    def _update_opacity_label(self, value: float) -> None:
        self.opacity_label.configure(
            text=f"{translate('widget_idle_opacity', self.language)}: {round(float(value) * 100 / 5) * 5:.0f}%"
        )
