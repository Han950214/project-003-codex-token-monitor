...
Phase 3.1-B2 为 Recommendation 增加可选的本地历史辅助证据，但不改变 Advisor v1 的阈值、优先级或 severity。历史查询失败、当前数据陈旧或样本不足时，结果会完全退化为原有 v1 规则。
...
统一状态中心只显示最高优先级建议，并且同时只有一个突出主操作；完整安全数字与相关说明可在指标详情、当前任务详情或诊断结果中按需查看。同一输入总是得到相同顺序和输出。
...
历史比较只读取 Token Monitor 自有趋势层中的安全数字，并要求同一 Thread、同一指标、非陈旧且可用。最低门槛集中定义为 5 个有效样本和 3 个不同可靠观测时间；满足后可显示相对近期中位值的上升、下降或大致持平。该结论仅是辅助说明，不预测未来额度、费用或质量，也不改变任何建议的排序。
