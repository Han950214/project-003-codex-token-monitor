...
- 标题：不再读取 State `threads.title`。完整刷新通过共享的 Codex `app-server` 连接执行一次 `thread/list` 批量请求，只解码结构化 `id` 与 `name`；`preview`、turn 和正文值会被跳过而不解码。UI 信任官方 `Thread.name`，不根据标题文字推断其是否为正文；仅缺失、无效、清理后为空、解析失败或未出现在批次中的名称使用安全时间回退。过长名称只在展示层截断。会话点击与翻页不发标题请求；瞬时标题请求失败时保留既有缓存。
